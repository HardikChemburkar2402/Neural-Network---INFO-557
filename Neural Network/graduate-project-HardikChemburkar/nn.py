...
def run_training(
    ...
    test_csv="test.csv",                         # (Optional) Testing dataset path
    batch_size=32,
    learning_rate=2e-5,
    epochs=10,
    dropout=0.4,
    ...
    # Load pre-trained tokenizer and transformer model
    tokenizer = transformers.AutoTokenizer.from_pretrained("distilroberta-base")
    base_model = transformers.TFAutoModel.from_pretrained("distilroberta-base")

    # Load training and validation data using HuggingFace datasets
    ...
    # Extract label values into a single "labels" column for each row
