...
"""
ISTA 457 / INFO 557 Spring 2025 HW 1
...
class Index:
    ...
    def __init__(self, vocab: Sequence[Any], start=0):
        ...
        for item in vocab:
            if item not in self.index_mapping:
                self.index_mapping[item] = current_index
                current_index += 1

        self.reverse_mapping = {v: k for k, v in self.index_mapping.items()}
        self.padding_value = start - 1
    ...
    def objects_to_indexes(self, object_seq: Sequence[Any]) -> np.ndarray:
        ...
        """
        return np.array(
            [self.index_mapping.get(obj, self.padding_value) for obj in object_seq],
            dtype=int
        )
    ...
    def objects_to_binary_vector(self, object_seq: Sequence[Any]) -> np.ndarray:
        ...
        """
        binary_vector = np.zeros(
            max(
                self.index_mapping.values(),
                default=self.padding_value) + 1,
                dtype=int)

        for obj in object_seq:
            index = self.index_mapping.get(
                obj,
                self.padding_value)
            if index >= self.start:
    ...
    def objects_to_binary_matrix(
        ...
        """
        return np.array(
            [self.objects_to_binary_vector(seq) for seq in object_seq_seq],
            dtype=int
        )
    ...
    def indexes_to_objects(self, index_vector: np.ndarray) -> Sequence[Any]:
        ...
        """
        return [
            self.reverse_mapping.get(index)
    ...
    def binary_vector_to_objects(self, vector: np.ndarray) -> Sequence[Any]:
        ...
        """
        return [self.reverse_mapping.get(index) for index in np.nonzero(vector)[0]]
